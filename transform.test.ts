...
describe('transform functions', () => {
  ...
  describe('having', () => {
    ...
    it('should filter groups with specific condition', () => {
      const groupByCity = groupBy('city');
      const groups = groupByCity(users);
      const filterGroups = having(group => group.items.some(u => u.age > 40));
      const result = filterGroups(groups);
  ...
  describe('valid order pipelines', () => {
    it('should work with where -> groupBy -> having', () => {
    ...
    it('should work with where -> where -> sort', () => {
      const pipeline = query<User>(
        where('name', 'John'),
        where('surname', 'Doe'),
        sort('age')
    ...
    it('should work with where -> sort', () => {
      const pipeline = query<User>(
        where('city', 'NY'),
        sort('age')
      );
    ...
    it('should work with groupBy -> having', () => {
      const pipeline = query<User>(
        groupBy('city'),
  ...
  describe('type system - invalid order should not compile', () => {
    it('should reject groupBy before where', () => {
      const invalid = query<User>(
        groupBy('city'),
        where('name', 'John')
      );
    });
